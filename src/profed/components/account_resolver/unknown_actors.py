...
async def _requested(object_id, payload, sequence_id) -> None:
    if gate.try_start(object_id):
        submit("unknown_actors", sequence_id, object_id)


async def resume() -> int:
    rows = [row for row in await (await storage()).unfinished() if gate.try_start(row["entry"])]
    for row in rows:
