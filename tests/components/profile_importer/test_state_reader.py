...
@pytest.mark.asyncio
async def test_avatar_changed_sets_reference(fake_bus):
    _users(fake_bus).messages = [_msg(1, "created", "alice", {"name": "Alice"}),
                                 _msg(2,
                                      "avatar_changed",
                                      "alice",
    ...
    async with state_reader.reading_state("alice") as (get_state, caught_up):
        ...
        profile = get_state()
    assert profile.avatar == MediaReference(media_id="m1", variants={"large", "small"})


@pytest.mark.asyncio
async def test_avatar_changed_empty_clears_avatar(fake_bus):
...
@pytest.mark.asyncio
async def test_avatar_changed_without_created_is_ignored(fake_bus):
    _users(fake_bus).messages = [_msg(1, "avatar_changed", "alice", {})]
...
async def test_get_state_is_usable_after_context_exit(fake_bus):
    ...
    assert get_state().name == "Alice"# Copyright (C) 2026 Christof Donat
# SPDX-License-Identifier: AGPL-3.0-or-later
