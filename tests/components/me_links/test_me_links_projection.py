...
def _payload(checked_at=NOW, stable_since=NOW, **rest):
    return dict({"profile_url": PROFILE,
...
async def test_a_verified_event_becomes_a_row(component):
    ...
    assert (await component.verification(ACTOR, LINK))["state"] == "verified"
...
async def test_an_unverified_event_becomes_a_row(component):
    ...
    assert (await component.verification(ACTOR, LINK))["state"] == "unverified"
...
async def test_a_gone_event_becomes_a_row(component):
    ...
    assert (await component.verification(ACTOR, LINK))["state"] == "gone"
...
async def test_the_freshness_headers_are_kept(component):
    ...
    stored = await component.verification(ACTOR, LINK)
...
async def test_a_fresh_page_is_due_after_the_minimum(component):
    ...
    assert (await component.verification(ACTOR, LINK))["next_due_at"] == NOW + timedelta(days=1)
...
async def test_a_stable_page_is_due_much_later(component):
    ...
    assert (await component.verification(ACTOR, LINK))["next_due_at"] > NOW + timedelta(days=15)
